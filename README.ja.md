...
- **自分のログを渡せる** — 端末レポートとrunnerログの末尾をクリップボードへ。
  releaseビルドは`run-as`を拒むので、手元にない端末が何が起きたかを語る手段はこれだけ
...
計測はしない」で、受理されたかだけを問う行に使います。`outputDir` も指定した行は、計測はしないまま
1回だけ実行されます — `outputFiles` のファイルには、実行が生んだものしか入りません。
...
| --- | --- |
| `0` | 実行された |
| `1` | そちら側の問題 — ロードできないモデル、合わない入力、またはリクエストに入れられないオプション値 |
| `2` | ドライバが断った — 記録して続行 |
| `3` | この端末にそのデバイスは無い |
| `4` | エージェントに到達できない — 中断 |
...
- [x] 誰が走らせたかだけでなく、なぜ断られたかを返す
      ([#158](https://github.com/m96-chan/DroidRunner/issues/158)) —
      **作ったのではなく、答えが出た**。TFLiteはXNNPACKのノード別理由は持つが
      NNAPI版を持たないので、NNAPIドライバが断った理由はここでは知りようがない。
      根拠つきで契約に明記した
- [ ] 2つのエンジンを跨ぐ代償を測る
