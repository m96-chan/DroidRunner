#!/usr/bin/env python3
"""Compares two operator matrices and fails on a regression — issue #126.
...
cloud machines cannot perform, and the sharpest form of what this project is
for.
...
def key(matrix):
    ...
    the drivers. A matrix taken before #118 has no `soc` at all, and the first
    pair this was pointed at were two runs of the same phone hours apart, one
    from either side of that: identifying by SoC called them different devices
    and refused to compare them.
    """
...
def main():
    ...
    if key(before) != key(after):
        ...
        # exit code has to be able to tell them apart.
        sys.exit(2)

    build_before = before.get("device", {}).get("droidrunner")
    build_after = after.get("device", {}).get("droidrunner")
    # A cell that moved between two DroidRunner builds may be our change and
    # not theirs, and saying so is the difference between a report someone acts
    # on and one they argue with (#124).
    ...
    for cell_key in sorted(set(old) | set(new)):
        ...
        if before_status in TAKEN and after_status not in TAKEN:
            ...
            # the right to fail somebody's build (#98).
    ...
    if not same_build:
        print(f"  builds differ: {build_before} -> {build_after} — "
              "a cell that moved may be ours and not theirs")
